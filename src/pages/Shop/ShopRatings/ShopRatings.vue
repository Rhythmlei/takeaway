        ...
        <div class="rating-type border-1px">
          ...
          >
            不满意<span class="count">{{
              shopRatings.length - positiveSize
            }}</span>
          </span>
        </div>
        <div
...
export default {
  data() {
    return {
      ...
      onlyShowText: true, // false：全部展示；           true：只展示有text的评论
    };
  },
  created() {
    this.$store.dispatch("receive_shopratings");
  },
  computed: {
    ...
    filterRatings() {
      return this.shopRatings.filter((rating) => {
        if (this.selectType === 2) {
          return !this.onlyShowText || rating.text.length > 0;
        } else if (this.selectType === rating.rateType) {
          return !this.onlyShowText || rating.text.length > 0;
        }
      });

      // 要selectType、onlyShowText两个条件，两个条件又各分两种情况，组合
      // 1.
      // selectType：2     onlyShowText：true   rating.text.length>0
      //                   onlyShowText:false   !onlyShowText:true

      // 2.
      // selectType：0/1   onlyShowText：true    rating.text.length>0
      //                   onlyShowText:false    !onlyShowText:true
    },
  },
  watch: {
